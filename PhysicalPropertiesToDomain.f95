...
subroutine rayleigh_extinct_old (nzt, Zlevels, Temp, wavelen, RaylExt)
...
end subroutine rayleigh_extinct_old
...
subroutine rayleigh_extinct (nzt, Zlevels, Temp, wavelen, RaylExt)
 ...
 ! The extinction profile is returned with zeros if wavelen<=0.
  ...
  integer :: i
  real    :: raylcoef, pres, lapse, ts, dz, extlev(nzt+1)
  real    :: P(nzt+1), R, g, M, C, Ps

  !total Rayleigh optical depth for a given wavelength
  !raylcoef = 0.0088*wavelen**(-4.15+0.2*wavelen)
  !total Rayleigh optical depth for a given wavelength
  !based on Hansen and Travis 1974 Eq.(2.30)
  raylcoef = 0.0085692*wavelen**(-4) * &
           (1 + 0.01132*wavelen**(-2) + 0.00013*wavelen**(-4))

   ! Find surface pressure by integrating hydrostatic relation
   !  for a dry atmosphere up to surface height.a
  R= 8.3144598 ![J/(mol·K)]
  g= 9.80665 ![m/s2]
  M = 0.0289644 ![kg/mol]
  lapse = 6.5*0.001 !lapse rate [K/m]
  C = -g*M/R/lapse

  pres = 1013.0
  ts = Temp(1)
  Ps = pres*(ts/(ts+lapse*Zlevels(1)*1000.))**(9.8/(287.*lapse))
  print*,'surface pressure',Ps
  P(1)=Ps
  extlev(1)=raylcoef

  do i = 1, nzt
     P(i+1) = Ps*((Temp(1)+lapse*(Zlevels(i+1)-Zlevels(1))*1000.0)/Temp(1))**C
     extlev(i+1)=raylcoef*P(i+1)/Ps
     print*,'level i', P(i+1),extlev(i+1)
  enddo
  do i = 1, nzt
    RaylExt(i) = (extlev(i)-extlev(i+1))/(Zlevels(i+1)-Zlevels(i))
  enddo
